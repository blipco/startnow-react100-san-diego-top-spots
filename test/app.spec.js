...
describe('top spots', function () {
  ...
  it('should show topspot name in each bootstrap card using a <h4> element.', () =>
    nightmare
    .goto(url)
    .wait('div.card')
    .evaluate(() => Array.prototype.slice.call(document.querySelectorAll('div.card>h4')).map(el => el.innerHTML))
    .end()
    .then((titles) => {
  ...
  it('should show topspot description in each bootstrap card using a <p> element.', () =>
    ...
    .goto(url)
    .wait('div.card')
    .evaluate(() => Array.prototype.slice.call(document.querySelectorAll('div.card>p')).map(el => el.innerHTML))
    .end()
    .then((descriptions) => {
  ...
  it('should show a link in each bootstrap card using an <a> element styled as a bootstrap button.', () =>
    ...
    .goto(url)
    .wait('div.card')
    .evaluate(() => Array.prototype.slice.call(document.querySelectorAll('div.card>a')).map(el => el.attributes.href.nodeValue))
    .end()
    .then((links) => {
